...
Note: Most of the figures in this report are vision deficiency friendly.
...
In this report of our data quality analysis and explratory analysis, Part A demonstrates results on price distribution, room type distribution and pet policy, while Part B shows the results from text analytics.  
...
### Analysis of data quality - review data
...
### Analysis of data quality - listing data of summary and review scores only
```{r}
library(dplyr)
ls <- listingsAll %>%
...
We created a new column by assigning each listing with a category based their review scores. The distribution of review scores is shown in ratings distribution of the Overview section using dataset from **listingsAll.csv** and is used as references to create 3 categories of ratings (A, B and C). The new column "rating_categ" was combined to the cleaned data to create **CleanedPlus.csv**. 
...
Key words of amenities description were extracted and counted. The words and frequencies of amenities were written to **amenities.csv** for the Amenities section. 
