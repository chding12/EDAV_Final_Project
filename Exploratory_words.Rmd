...
## I. Overview
### missing pattern in review data
...
#all
wordcloud(words = adj_s_all$word, freq = adj_s_all$freq,
                      min.freq = 1,
...
### Are words of summary in each rating category
```{r fig.height=8, fig.width=8}
library(ggplot2)
...
### Is the length of reviews of different room-types and boroughs different?
```{r}
...
### Does the length of review changes when people give good or bad reviews on each type of rooms? 
```{r fig.width=5, fig.height=5}
library(grid)
r2 <- gl %>%
...
#### weekly patterns in different years
```{r}
